...
module testbench;
    ...
    initial begin
        ...
        real A = 1; // Amplitude (V) - original definition
        ...
        for (int i = 0; i < num_samples; i++) begin
            ...
            delta = (adc_data + prev_adc_data) / 2;          
            tb_integral_sum += delta;
    ...
    final begin
        ...
        $display("Trapezoidal Integral (TB): %0e V*s, raw: %0d", tb_integral_sum * (5.0 / 4095.0) * CLK_PERIOD_REAL * 1e-9, tb_integral_sum);
        $display("Trapezoidal Integral (DUT): %0e V*s, raw: %0d", integral_value * (5.0 / 4095.0) * CLK_PERIOD_REAL * 1e-9, integral_value); // Display DUT's final integral
