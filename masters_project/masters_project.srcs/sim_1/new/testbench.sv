...
module testbench;
    ...
    initial begin
        ...
        for (int i = 0; i < num_samples; i++) begin
            ...
            $display("Iteratio: %0d, Time: %0t, current_time: %0f, pulse_val: %0f, scaled_adc_data_real: %0f, adc_data: %h", i, $time, current_time_local, pulse_val, (pulse_val / 5.0) * 4095.0, adc_data);
            
            // Accumulate for non-interpolated integral (for verification)
            tb_non_interp_integral_sum += adc_data;
            prev_adc_data = adc_data;
            tb_interp_integral_sum += (prev_adc_data * 4 + ((adc_data - prev_adc_data) * 6 / 4));
            #(CLK_PERIOD);
    ...
    final begin
        ...
        $display("Interpolated Integral (DUT): %0e V*s, raw: %0d", integral_value * (5.0 / 4095.0) * CLK_PERIOD_REAL * 1e-9 / 4, integral_value); // Display DUT's final integral
